...
class UploadBlksLinkView(APIView):
    ...
    def get(self, request, repo_id, format=None):
        ...
        token = seafile_api.get_fileserver_access_token(repo_id,
                                                        'dummy', 'upload-blks-api', request.user.username,
                                                        use_onetime=False)
...
class UpdateBlksLinkView(APIView):
    ...
    def get(self, request, repo_id, format=None):
        ...
        token = seafile_api.get_fileserver_access_token(repo_id,
                                                        'dummy', 'update-blks-api', request.user.username,
                                                        use_onetime=False)
...
class FileView(APIView):
    ...
    def post(self, request, repo_id, format=None):
        ...
        if operation.lower() == 'rename':
            ...
            if dst_dir[-1] != '/':
                # Append '/' to the end of directory if necessary
                dst_dir += '/'
...
class FileHistory(APIView):
    ...
    def get(self, request, repo_id, format=None):
        ...
        return HttpResponse(json.dumps({"commits": commits}, cls=SearpcObjEncoder), status=200,
                            content_type=json_content_type)
...
class SharedLinksView(APIView):
    ...
    def get(self, request, format=None):
        ...
        return HttpResponse(json.dumps({"fileshares": p_fileshares}, cls=FileShareEncoder), status=200,
                            content_type=json_content_type)
...
class Groups(APIView):
    ...
    def put(self, request, format=None):
        ...
        if not validate_group_name(group_name):
            result[
                'error'] = 'Failed to rename group, group name can only contain letters, numbers, blank, hyphen or underscore.'
...
class OrganizationView(APIView):
    ...
    def post(self, request, format=None):
        ...
        if not slug_re.match(prefix):
            return api_error(status.HTTP_400_BAD_REQUEST,
                             "URL prefix can only be letters(a-z), numbers, and the underscore character")
